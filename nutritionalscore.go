...
type StaturatedFattyAcids float64
...
type NutritionalData struct{
	Energy					EnergyKJ
	Sugars					SugarGram
	StaturatedFattyAcids	StaturatedFattyAcids
...
func (sfa StaturatedFattyAcids) GetPoints(st ScoreType) int {
...
func GetNutritionalScore(n NutritionalData, st ScoreType) NutritionalScore{
	...
	if st != Water {
		...
		negative = n.Energy.GetPoints(st) + n.Sugars.GetPoints(st) + n.StaturatedFattyAcids.GetPoints(st) + n.Sodium.GetPoints(st)
